...
#pragma omp parallel for
  for (int i = 0; i < rowMax; i++) {
    Diagonals<T> A = createCoeffMatrix(alphaX, bcLeft, bcRight, colMax, i, sx);
    Eigen::VectorX<T> b =
...
#pragma omp parallel for
  for (int i = 0; i < colMax; i++) {
    ...
    Diagonals<T> A = createCoeffMatrix(alphaY, bcTop, bcBottom, rowMax, i, sy);
    Eigen::VectorX<T> b =
        createSolutionVector(concentrations_t1, alphaY, alphaX, bcTop, bcBottom,
